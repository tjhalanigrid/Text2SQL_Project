...
def get_allowed_tokens_from_schema(schema_str):
    """
    Extract valid table + column tokens from schema
    """
    tokens = set()

    schema_str = schema_str.lower()
...
def apply_schema_constraint(sql, allowed_tokens):
    """
    Filter SQL tokens using schema-aware constraints
    """
    tokens = sql.lower().split()

    filtered = []
    for t in tokens:
        # Keep SQL keywords OR valid schema tokens
        if t in allowed_tokens or not t.isidentifier():
            filtered.append(t)
...
ALLOWED_TOKENS = get_allowed_tokens_from_schema(SCHEMA)
...
"""
    ...
    sql = apply_schema_constraint(sql, ALLOWED_TOKENS)
