...
# # =========================
# # TASK 2: ERROR CLASSIFICATION & HINTS
# # =========================
# def classify_error(sql):
#     sql = sql.lower()

#     if "join" in sql and " on " not in sql:
#         return "missing_join"
#     if "avg(" in sql and ")" not in sql:
...
#     # Resolve DB path dynamically (FIXED: Added the missing db_id sub-folder)
...
#     try:
#         valid, _ = validate_sql_schema(final_sql, db_path)
#     except:
#         valid = False

#     # ==========================================
#     # TASK 2 & 3: VALIDATION AND HINT INJECTION
#     # ==========================================
...
def _load_attribution_model(base_model: str, adapter: str, device: str):
    ...
    if key in _ATTRIBUTION_CACHE: return _ATTRIBUTION_CACHE[key]
...
def gradient_attribution_for_error(base_model: str, adapter: str, db_id: str, question: str, sql: str, top_k: int = 12) -> str:
    device = "mps" if torch.backends.mps.is_available() else ("cuda" if torch.cuda.is_available() else "cpu")
    try: tokenizer, model = _load_attribution_model(base_model, adapter, device)
    except Exception as e: return f"Model load failed: {e}"

    try: schema = schema_encoder.structured_schema(str(db_id))
    ...
    if out.loss is None: return "No loss computed."
    out.loss.backward()

    if inputs_embeds.grad is None: return "No gradients."
    scores = torch.norm(inputs_embeds.grad[0], dim=-1).detach().float().cpu()
    toks = tokenizer.convert_ids_to_tokens(enc.input_ids[0].detach().cpu().tolist())
    pairs = sorted(list(zip(range(len(toks)), toks, scores.tolist())), key=lambda x: x[2], reverse=True)[: int(top_k)]

    lines = [f"loss={float(out.loss.detach().cpu()):.4f}", "Top prompt tokens by gradient norm:"]
    lines.extend([f"- {idx}: {tok} -> {sc:.6f}" for idx, tok, sc in pairs])
    return "\n".join(lines).strip()
...
with gr.Blocks(theme=gr.themes.Soft(), title="Text-to-SQL RLHF") as demo:
    ...
    with gr.Tabs():
        ...
        with gr.Tab("Diagnostics"):
            ...
            with gr.Accordion("Task 2: Adversarial suite", open=False):
                ...
                adv_run.click(fn=run_adversarial_suite, inputs=[quant_artifact_dir, use_constrained_decoding, gen_beams, gen_max_new, exec_timeout_s, exec_workers, exec_cache_on], outputs=[adv_out, adv_summary])
            ...
            with gr.Accordion("Task 2: Gradient attribution (slow)", open=False):
                ...
                def _populate_types():
                    if not _QUERY_LOG: return gr.update(choices=[], value=None)
                    counts = {}
                    for r in _QUERY_LOG[-1000:]:
                        k = r.get("error_type") or "other"
                        counts[k] = counts.get(k, 0) + 1
                ...
                attr_run.click(fn=lambda b,a,t,k: gradient_attribution_for_error(str(b), str(a), str([r for r in reversed(_QUERY_LOG) if (r.get("error_type") or "") == str(t)][0].get("db_id", "")) if [r for r in reversed(_QUERY_LOG) if (r.get("error_type") or "") == str(t)] else "", str([r for r in reversed(_QUERY_LOG) if (r.get("error_type") or "") == str(t)][0].get("question", "")) if [r for r in reversed(_QUERY_LOG) if (r.get("error_type") or "") == str(t)] else "", str([r for r in reversed(_QUERY_LOG) if (r.get("error_type") or "") == str(t)][0].get("sql", "")) if [r for r in reversed(_QUERY_LOG) if (r.get("error_type") or "") == str(t)] else "", int(k)) if t else "Pick an error type.", inputs=[attr_base, attr_adapter, attr_type, attr_topk], outputs=[attr_out])
    ...
    sample_dropdown.change(fn=load_sample, inputs=[sample_dropdown], outputs=[db_id])
    db_id.change(fn=update_schema, inputs=[db_id], outputs=[schema_display])
    run_btn.click(fn=run_query, inputs=[input_method, sample_dropdown, custom_question, db_id, quant_artifact_dir, use_constrained_decoding, gen_beams, gen_max_new, exec_timeout_s, exec_workers, exec_cache_on], outputs=[final_sql, result_table, explanation])
