...
#     # ==========================================
#     # 🔥 STABLE GENERATION (DO NOT CHANGE)
#     # ==========================================
...
#     # ==========================================
#     # 🔥 FINAL OPTIMIZED ASK (KEY IMPROVEMENT)
#     # ==========================================
...
#         sql1 = self.generate_sql(prompt)
#         sql2 = self.generate_sql(prompt)

#         # apply FK fix
#         sql1 = fix_joins_with_fk(sql1, str(db_path))
#         sql2 = fix_joins_with_fk(sql2, str(db_path))
...
#         valid1 = is_valid(sql1)
#         valid2 = is_valid(sql2)

#         # 🔥 SELECT BEST SQL
#         if valid1 and valid2:
#             sql = sql1 if len(sql1) >= len(sql2) else sql2
#         elif valid1:
#             sql = sql1
#         elif valid2:
#             sql = sql2
...
#             retry_sql = self.generate_sql(retry_prompt)

#             if is_valid(retry_sql):
#                 sql = retry_sql
...
# # ==========================================
# def get_engine(adapter_path, use_constrained=True):
#     return Text2SQLEngine(adapter_path)
...
# # ==========================================
# # INPUT VALIDATION & RELEVANCE (THE CAT BLOCKER)
# # ==========================================
...
# # ==========================================
# def normalize_question(q):
#     # Preserves case for things like 'Rock'
...
# def fix_joins_with_fk(sql, db_path):
#     # Safely fixes missing ON clauses using Foreign Keys
#     fks = get_foreign_keys(db_path)
#     sql_lower = sql.lower()
#     for table, col, ref_table, ref_col in fks:
#         if table in sql_lower and ref_table in sql_lower:
...
#         # 1. Normalize without losing capital letters
#         question_norm = normalize_question(question)
#         schema = self.get_schema(db_id)
        
#         # 2. Domain Guardrails (The Cat Blocker)
#         schema_graph = build_schema_graph(schema)
#         if not is_relevant_to_db(question_norm, schema_graph):
#             return {"sql": "", "columns": [], "rows": [], "error": "❌ Question is completely out of domain for the selected database."}

#         prompt = self.build_prompt(question_norm, schema)

#         # 3. Robust DB Path Finder
#         db_path = DB_ROOT / db_id / f"{db_id}.sqlite"
...
#         if not is_valid(sql):
#             retry_prompt = f"Fix SQL using schema.\nSQL:\n{sql}\nSchema:\n{schema}"
#             retry_sql = self.generate_sql(retry_prompt)
#             if is_valid(retry_sql):
#                 sql = retry_sql

#         # 5. Semantic Fix (LIMIT & Aggressive Hallucination Cleanup)
#         sql = semantic_fix(question_norm, sql)
...
#     return Text2SQLEngine(adapter_path, base_model_name=base_model_name, use_lora=use_lora)
